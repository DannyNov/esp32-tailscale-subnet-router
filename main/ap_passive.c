...
static void learn_on_tcpip(void *arg)
{
    ...
    if (n && n == o->netif && !mac_deny_is_blocked(o->mac) &&
        esp_wifi_ap_get_sta_list(&stations) == ESP_OK) {
        for (int i = 0; i < stations.num; ++i)
            if (!memcmp(stations.sta[i].mac, o->mac, 6)) associated = true;
    }
    ...
    if (associated) {
        ...
        if (etharp_find_addr(n, &ip, &owner, &found_ip) >= 0 &&
            owner && memcmp(owner->addr, o->mac, 6)) associated = false;
    }
    uint32_t previous = dhcp_reservations_lookup(o->mac);
    if (dhcp_reservations_observe(o->mac, o->ip, associated)) {
        ip4_addr_t ip = {.addr = o->ip};
        struct eth_addr mac;
        memcpy(mac.addr, o->mac, 6);
        err_t err = tsr_etharp_add_static_entry(n, &ip, &mac);
        if (!previous) ESP_LOGW("ap_passive", "learned %02x:%02x:%02x:%02x:%02x:%02x -> "
                               IPSTR " (NVS committed, AP ARP result=%d)",
